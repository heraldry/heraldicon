(ns heraldicon.frontend.entity.action.favorite
  (:require
   [cljs.core.async :refer [go]]
   [com.wsscode.async.async-cljs :refer [<?]]
   [heraldicon.frontend.api :as api]
   [heraldicon.frontend.entity.core :as entity]
   [heraldicon.frontend.entity.form :as form]
   [heraldicon.frontend.language :refer [tr]]
   [heraldicon.frontend.user.session :as session]
   [re-frame.core :as rf]
   [taoensso.timbre :as log]))

(rf/reg-event-db ::update-favorite
  (fn [db [_ form-db-path {:keys [is-user-favorite
                                  favorite-count]}]]
    (-> db
        (assoc-in (conj form-db-path :favorite-count) favorite-count)
        (assoc-in (conj form-db-path :is-user-favorite) is-user-favorite))))

(rf/reg-fx ::toggle-favorite
  (fn [[form-db-path entity-data session]]
    (go
      (try
        (let [;; TODO: this can probably live elsewhere
              payload {:entity-id (:id entity-data)}
              result (<? (api/call :toggle-favorite payload session))]
          (rf/dispatch [::update-favorite form-db-path result]))

        (catch :default e
          (log/error e "favorite error"))))))

(rf/reg-event-fx ::invoke
  (fn [{:keys [db]} [_ entity-type session]]
    (let [form-db-path (form/data-path entity-type)
          entity-data (get-in db form-db-path)]
      {::toggle-favorite [form-db-path entity-data session]})))

(defn action [entity-type]
  (let [form-db-path (form/data-path entity-type)
        logged-in? @(rf/subscribe [::session/logged-in?])
        can-favorite? (and logged-in?
                           @(rf/subscribe [::entity/saved? form-db-path]))
        session @(rf/subscribe [::session/data])]
    {:title :string.button/share
     :handler (when can-favorite?
                #(rf/dispatch [::invoke entity-type session]))
     :disabled? (not can-favorite?)
     :tooltip (when-not logged-in?
                :string.user.message/need-to-be-logged-in)}))

(defn button [entity-type & {:keys [height]
                             :or {height 24}}]
  (let [{:keys [handler
                disabled?
                tooltip]} (action entity-type)
        form-db-path (form/data-path entity-type)
        favorite-count @(rf/subscribe [:get (conj form-db-path :favorite-count)])
        is-favorite? @(rf/subscribe [:get (conj form-db-path :is-user-favorite)])]
    [:button.button.favorite
     {:style {:flex "initial"
              :padding "5px"
              :line-height (str height "px")}
      :class (when disabled? "disabled")
      :title (tr tooltip)
      :on-click handler}
     [:svg {:version "1.1"
            :xmlns "http://www.w3.org/2000/svg"
            :viewBox "-1 -1 27 25"
            :style {:height (str height "px")
                    :vertical-align "top"}}
      [:path {:d "M 17.736302,0
                  C 21.748672,0 25,3.2738716 25,7.3140594 25,9.6415126 23.444994,11.863869 22.298625,13.114203
                  L 12.499998,23 2.7013743,13.114203
                  C 1.5550053,11.863869 0,9.6415126 0,7.3140594 0,3.2738716 3.2513274,0 7.2636967,0 9.3201851,0 11.178305,0.85924472 12.499998,2.2420263 13.821695,0.85924472 15.679814,0 17.736302,0
                  Z"
              :style {:stroke-width 1
                      :stroke "#000"
                      :fill (if is-favorite?
                              "red"
                              "none")}}]]
     [:div {:style {:display "inline-block"
                    :padding-left "5px"
                    :font-size (str (* height 0.8) "px")
                    :vertical-align "top"}}
      favorite-count]]))
