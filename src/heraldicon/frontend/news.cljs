...
(def posts
  [{:id "2026-04-03"
   ...
   {:id "2025-01-01-ontology"
    :date "2025-01-01"
    :title "Charge type ontology and better search"
    :content
    (fn []
      [[:div.release-row
        [:div.info
   ...
   {:id "2024-02-25"
    :title "Drag and drop for components and collection arms"
    :content
    (fn []
      [[:div.release-row
        [:div.info
   ...
   {:id "2024-01-26"
    :title "Vairy/lozengy/potenty/papellony/masony rendering improved"
    :content
    (fn []
      [[:div.release-row
        [:div.info
   ...
   {:id "2023-03-19"
    :title "Bordure/orle line style adjustments and flory line styles"
    :content
    (fn []
      [[:div.release-row
        [:div.info
   ...
   {:id "2022-09-01"
    :title "New rendering engine and various improvements"
    :content
    (fn []
      [[:div.release-row
        [:div.info
         ...
         [:ul
          [:li [:b "new rendering engine"] " - this is the biggest change, albeit largely invisible, some immediate differences are:"
           [:ul
            ...
            [:li [:b "some of these changes are not fully backwards compatible"] ", so if you see changes in your arms, especially in partitions, ordinaries, cottises, line styles, or fimbriation, then that may be why; I try to migrate data, but sometimes that's not feasible... and hopefully these cases can be fixed easily"]]]
          ...
          [:li "new anchor/orientation points:"
           [:ul
            [:li [:b "center"] " - the exact middle of the field's bounding box, often coincides with 'fess' on subfields"]
            [:li [:b "hoist"] " - a point left of the center for flags, similar to 'fess' in arms"]
            [:li [:b "fly"] " - a point right of the center for flags, opposite 'hoist'"]]]
   ...
   {:id "2022-04-14"
    :title "Change of 'origin'/'anchor' naming"
    :content
    (fn []
      [[:div.release-row
        [:div.info
         ...
         [:ul
          [:li [:b "anchor"] " (previously " [:em "origin"] "): is the main point an element is positioned with."]
          [:li [:b "orientation"] " (previously " [:em "anchor"] "): is the point an element orients itself towards, but it can also be a fixed angle."]]
   ...
   {:id "2022-04-07"
    :title "Blazonry reader, translations, bordures with line styles, metadata"
    :content
    (fn []
      [[:div.release-row
        [:div.info
   ...
   {:id "2021-11-25"
    :title "Bordure/orle, in orle charge groups, chevronny"
    :content
    (fn []
      [[:div.release-row
        [:div.info
   ...
   {:id "2021-10-25"
    :title "Supporters, compartments, mantling, charge library improvements"
    :content
    (fn []
      [[:div.release-row
        [:div.info
   ...
   {:id "2021-09-26"
    :title "German translation, undo/redo, quarter/canton/point ordinaries, fretty"
    :content
    (fn []
      [[:div.release-row
        [:div.info
   ...
   {:id "2021-08-20"
    :title "Helms/crests, mottos/slogans, ribbon editor + library"
    :content
    (fn []
      [[:div.release-row
        [:div.info
   ...
   {:id "2021-07-30"
    :title "UI rewrite, validation system, social media sharing"
    :content
    (fn []
      [[:div.release-row
        [:div.info
   ...
   {:id "2021-06-12"
    :title "Charge groups, public arms, collections, users"
    :content
    (fn []
      [[:div.release-row
        [:div.info
   ...
   {:id "2021-03-31"
    :title "Embowed/enarched, nonrepeating line styles"
    :content
    (fn []
      [[:div.release-row
        [:div.info
